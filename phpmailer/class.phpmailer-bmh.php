...
$hostname_datab = "localhost";
$database_datab = "ipsc_email";
$username_datab = "ipsc_email";
$password_datab = "ipsc_email0404";

$datab = mysql_connect($hostname_datab,$username_datab,$password_datab) or die(mysql_error()); 
mysql_select_db($database_datab, $datab)or die("Error");
	...
	// CR001 Ends	
      ...
      */
      ...
      if ($processed) {
		...
		if ( ($this->testmode === false) && ($this->bounced_email_id != "") ) {
// Added   by sanjeev for Inserting Bounce Back Email IDs in Database.

			//echo "Bounced mail id : " . $this->bounced_email_id . "<br>";
			$mail_log_query = "SELECT * FROM bouncebackemails WHERE subject='$this->subject' AND mail='$this->bounced_email_id'"; 
			$log_mail = mysql_query($mail_log_query);
			$bounced_mails_found = mysql_num_rows($log_mail);
			//echo $bounced_mails_found;

			if ($bounced_mails_found == 0) {
				$mail_log_query = "INSERT INTO bouncebackemails (subject,mail) 
											VALUES ('$this->subject','$this->bounced_email_id')"; 
				$log_mail = mysql_query($mail_log_query);
				if (!$log_mail) {
					echo "Could not successfully run query ($mail_log_query) from PHP Mailer - BMH: " . mysql_error();
					//exit;
				}
			}
