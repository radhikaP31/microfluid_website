...
//	echo $body;
   ...
   */
  elseif (preg_match ("/(\S+@\S+\w).*\n?.*no such (?:user|address) here/i",$body,$match)) {
 ...
 */
  ...
  /* rule: unknown
 * sample 1:
 * Diagnostic-Code: SMTP; 5.4.4 xxxxx@yourdomain.com:user not exist
 * sample 2:
 * Diagnostic-Code: SMTP; 5.4.4 sorry, that recipient doesn't exist (#5.7.1)
 */
	elseif (preg_match ("/(?:host|domain).*(?:n't|not) found/is",$body,$match)) {
	  $result['rule_cat']    = 'unknown';
	  $result['rule_no']     = '0204';
      $result['email']       = $match[1];
	}
  ...
  elseif (preg_match ("/(\S+@\S+\w)<br>.*\n?.*\n?.*can't find.*mailbox/i",$body,$match)) {
	...
	// CR001 Starts
	/* rule: user doesn't have account
	 * sample:This user doesn't have a yahoo.com account (****@****.com)
	 * Diagnostic-Code: SMTP; 554 delivery error
	 */
	elseif (preg_match ("/This user doesn't have a .* account/i",$body,$match)) {
	  $result['rule_cat']    = 'error';
	  $result['rule_no']     = '0164';
	  $result['email']       = $match[1];	  
	}
	
	/* rule: over quota
	 * sample:Reason: Over quota
	 * Diagnostic-Code:
	 */
	...
	elseif (preg_match ("/Mailbox quota.*exceeded/i",$body,$match)) {
	  $result['rule_cat']    = 'error';
	  $result['rule_no']     = '0164';
	  $result['email']       = $match[1];	  
	}		
	
	/* rule: Message too large
	 * sample:Reason: Message too large
	 * Diagnostic-Code:
	 */
	elseif (preg_match ("/Message too large/i",$body,$match)) {
	  $result['rule_cat']    = 'error';
	  $result['rule_no']     = '0164';
	  $result['email']       = $match[1];	  
	}		
	 ...
	 * Diagnostic-Code:
	 */
	elseif (preg_match ("/Connection timed out/i",$body,$match)) {
	  $result['rule_cat']    = 'error';
	  $result['rule_no']     = '0164';
	  $result['email']       = $match[1];	  
	}		
   ...
   * local: Sorry, can't find user's mailbox. (#5.1.1)<br>
   */
  elseif (preg_match ("/mailbox unavailable/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0164';
    $result['email']       = $match[1];
  }

  elseif (preg_match ("/Address rejected./i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0164';
    $result['email']       = $match[1];
  }
   ...
   */
  elseif (preg_match ("/(\S+@\S+\w).*\n?(.*\n)?(.*\n)?.*retry timeout exceeded/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0232';
    $result['email']       = $match[1];
  }
  ...
  /*
   * rule: retry time not reached for any host after a long failure period
   * sample:
   * A message that you sent could not be delivered to one or more of its^M
   * recipients. This is a permanent error. The following address(es) failed:^M
   * ^M
   ...
   */
  elseif (preg_match ("/(\S+@\S+\w).*\n?(.*\n)?(.*\n)?.*retry time not reached for any host after a long failure period/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0232';
    $result['email']       = $match[1];
  }
   ...
   * ^M
   * xxxxx@yourdomain.com^M
   * SMTP error from remote mail server after RCPT TO:<xxxxx@yourdomain.com>:
   * host yourdomain.com [*.*.*.*]: 450 Recipient address rejected:
   * Greylisting in action, please try later: retry timeout exceeded
   */
  ...
  /*
   * rule: The address to which the message has not yet been delivered is:
   * sample:
   * A message that you sent has not yet been delivered to one or more of its^M
   * recipients after more than 48 hours on the queue on yourdomain.com.^M
   * ^M
   *
   ...
   * xxxxx@yourdomain.com^M
   */
  elseif (preg_match ("/The address to which the message has not yet been delivered is/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0232';
    $result['email']       = $match[1];
  }
  ...
  if (empty($result['email'])) {
    ...
    switch ($action) {
      case 'failed':
        ...
        /* rule: inactive
         * sample:
         * Diagnostic-Code: SMTP; 550 xxxxx@yourdomain.com Mailbox is inactive
         */
        elseif (preg_match ("/(?:alias|account|recipient|address|email|mailbox|Mailbox|user) is inactive/is",$diag_code)) {
          $result['rule_cat']    = 'inactive';
          $result['rule_no']     = '0155';
        }
		// CR001 Ends		
		...
		// CR001 Starts
		  /* rule: unknown
		 * sample 1:
		 * Diagnostic-Code: SMTP; 5.4.4 xxxxx@yourdomain.com:Host or domain name not found
		 * sample 2:
		 * Diagnostic-Code: SMTP; 5.4.4 Host not found
		 */
			elseif (preg_match ("/(?:host|domain).*(?:n't|not) found/is",$diag_code)) {
			  $result['rule_cat']    = 'unknown';
			  $result['rule_no']     = '0204';
			}
         ...
         * Diagnostic-Code: SMTP; 554 delivery error
         */
        elseif (preg_match ("/This user doesn't have a .* account/is",$diag_code)) {
          $result['rule_cat']    = 'error';
          $result['rule_no']     = '0188';
        }		
		
        elseif (preg_match ("/Mailbox quota.*exceeded/is",$diag_code)) {
          $result['rule_cat']    = 'error';
          $result['rule_no']     = '0188';
        }		

        elseif (preg_match ("/Address rejected\./is",$diag_code)) {
          $result['rule_cat']    = 'error';
          $result['rule_no']     = '0188';
        }		
         ...
         * Diagnostic-Code: SMTP; 554 delivery error
         */
        elseif (preg_match ("/Connection timed out/is",$diag_code)) {
          $result['rule_cat']    = 'error';
          $result['rule_no']     = '0188';
        }		
        ...
        /* rule: Message too large
         * sample:Message too large
         * Diagnostic-Code: 
         */
        elseif (preg_match ("/Message too large/is",$diag_code)) {
          $result['rule_cat']    = 'error';
          $result['rule_no']     = '0188';
        }
